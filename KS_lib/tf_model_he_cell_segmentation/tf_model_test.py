...
def batch_processing(filename, sess, logits_test, parameters, images_test, keep_prob, mean_image, variance_image, flags):
    ...
    ori_image = KSimage.imread(filename)
    ori_image = KSimage.deconvolveHE(ori_image)
    grey = KSimage.rgb2gray(ori_image)
    thresh = threshold_otsu(grey)
    nuclear_mask = KSimage.imdilate(KSimage.bwareaopen(grey < thresh, 10**2), 20)
    nuclear_mask = np.logical_not(KSimage.bwareaopen(np.logical_not(nuclear_mask), 10**2))
    im_out = KSimage.bwareaopen(np.logical_and(nuclear_mask, im_out), 10**2)
