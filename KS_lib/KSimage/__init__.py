...
def deconvolveHDAB(I):
    ...
    J = np.reshape(I, (-1, 3))
    OD = -np.log((J + 1.0) / I0)
    C = np.dot(OD, hed_from_rgb)

    H = I0 * np.exp(np.dot(np.reshape(C[:, 0], (-1, 1)), -np.reshape(rgb_from_hed[0, :], (1, -1))))
    H = np.reshape(H, (h, w, c))
    H = np.clip(H, 0, 255)
    H = H.astype(np.uint8)

    E = I0 * np.exp(
        np.dot(np.reshape(C[:, 1], (-1, 1), order='F'), -np.reshape(rgb_from_hed[1, :], (1, -1), order='F')))
    E = np.reshape(E, (h, w, c), order='F')
    E = np.clip(E, 0, 255)
    E = E.astype(np.uint8)

    Bg = I0 * np.exp(
        np.dot(np.reshape(C[:, 2], (-1, 1), order='F'), -np.reshape(rgb_from_hed[2, :], (1, -1), order='F')))
    Bg = np.reshape(Bg, (h, w, c), order='F')
    Bg = np.clip(Bg, 0, 255)
    Bg = Bg.astype(np.uint8)
...
#############################################################
def registration_corr2_translation_rotation(moving, fixed):
    scale = 4
    ...
    rows, cols = moving_small.shape
    moving_small_no_rotate = np.copy(moving_small)
    all_theta = range(-90, 90, 1)

    ypeak = np.zeros(len(all_theta))
    xpeak = np.zeros(len(all_theta))
    minL = np.zeros(len(all_theta))

    for idx, theta in enumerate(all_theta):
        M = cv2.getRotationMatrix2D((cols / 2, rows / 2), theta, 1)
        moving_small = cv2.warpAffine(moving_small_no_rotate, M, (cols, rows))
        ...
        L[PCImage < np.amax(PCImage) * 0.3] = 0
        minL[idx] = L.min()
        ypeak[idx] = np.unravel_index(L.argmin(), L.shape)[0]
        xpeak[idx] = np.unravel_index(L.argmin(), L.shape)[1]
    ...
    theta = all_theta[min_idx]
    rows, cols = moving.shape
    M_coarse_rotate = cv2.getRotationMatrix2D((cols / 2, rows / 2), theta, 1)
    moving_coarse = cv2.warpAffine(moving, M_coarse_rotate, (cols, rows))

    M_coarse = np.float32([[1, 0, xoffSet], [0, 1, yoffSet]])
    moving_coarse = cv2.warpAffine(moving_coarse, M_coarse, (moving.shape[1], moving.shape[0]))
    ...
    M = np.vstack((M, [0, 0, 1]))
    M_coarse_rotate = np.vstack((M_coarse_rotate, [0, 0, 1]))

    M = np.matmul(M, M_coarse_rotate)
    M = M[0:2, ]
...
#############################################################
def register_CK2HE_translation(ck, he):
    sigma = 5
    kernel_size = 6 * sigma - 1
    ...
    Hck, _, _ = deconvolveHDAB(ck)
    Hck = rgb2gray(Hck)
    Hck = 255 - Hck
    Hck = cv2.GaussianBlur(Hck, (kernel_size, kernel_size), 0)

    Transformation = registration_corr2_translation(Hck, H)
    new_ck = cv2.warpAffine(ck, Transformation, (ck.shape[1], ck.shape[0]))
...
#############################################################
def register_CK2HE_translation_rotation(ck, he):
    sigma = 5
    kernel_size = 6 * sigma - 1
    ...
    Transformation, minL = registration_corr2_translation_rotation(Hck, H)
    new_ck = cv2.warpAffine(ck, Transformation, (ck.shape[1], ck.shape[0]),
    ...
    return new_ck, minL, status


#############################################################
def register_CK2FISH_translation_rotation(ck, fish):
    sigma = 5
    kernel_size = 6 * sigma - 1

    H, _, _ = deconvolveHDAB(ck)
    H = rgb2gray(H)
    H = 255 - H
    H = cv2.GaussianBlur(H, (kernel_size, kernel_size), 0)
    ...
    Transformation, minL = registration_corr2_translation_rotation(H, DAPI)
    new_ck = cv2.warpAffine(ck, Transformation, (ck.shape[1], ck.shape[0]))
...
#############################################################
def register_HE2FISH_translation_rotation(he, mask, fish):
    sigma = 5
    kernel_size = 6 * sigma - 1
    ...
    return new_he, new_mask, minL, status
